...
SINGLE_LANE_DETECTION_CENTERING_OFFSET = 0.4
OFFSET_CONSTANT = 1
MAX_OFFSET = 1
ANGLE_CONSTANT =0.8 
...
def image_callback(data):
    ...
    cv_image = cv.GaussianBlur(cv_image,(7,7),cv.BORDER_DEFAULT)
    cv_image = cv.GaussianBlur(cv_image,(7,7),cv.BORDER_DEFAULT)
    # cv_image = equalize_brightness(cv_image)
    cv_image = cv_image[MID_WAY_CROP:480][...]
    hsv_image = cv.cvtColor(cv_image,cv.COLOR_RGB2HSV)
    range_mat = cv.inRange(hsv_image,np.array([H_MIN,S_MIN,V_MIN]),np.array([H_MAX,S_MAX,V_MAX]))
    gaussian_blur = cv.GaussianBlur(range_mat,(7,7),cv.BORDER_DEFAULT)
    canny_edge = cv.Canny(gaussian_blur,100,200)
    display_image = cv.cvtColor(cv_image,cv.COLOR_BGR2RGB)
    cv_image = cv_image[TOP_CROP-MID_WAY_CROP:480-MID_WAY_CROP][...]
...
# Display global lines 
    ...
    #cv.imshow(winname=slider_window, mat=canny_edge)
    #cv.imshow(winname=display_window, mat=display_image)
    #cv.imshow(winname=birds_eye_window,mat=global_mat)
    #cv.imshow(winname=in_range_window,mat=gaussian_blur)
    cv.waitKey(5)
    # Send Drive Command
...
def equalize_brightness(image):
    ...
    max_val = HSV[2:3][...].max()
    adjustment = 255 - max_val
    test = HSV[2:3][...] + adjustment
    HSV[2:3][...] = HSV[2:3][...] + adjustment
