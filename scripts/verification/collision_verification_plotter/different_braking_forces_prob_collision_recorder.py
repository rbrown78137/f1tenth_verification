...
breaking_force_labels = ["-1.6 m/s^2","-0.8 m/s^2","-0.4 m/s^2","-0.2 m/s^2","-0 m/s^2"]
...
global_video_writer = cv.VideoWriter('/home/ryan/Paper_Video/different_braking_forces.avi', 
...
def process_reachability_step_thread():
    while(True):
        ...
        if len(copy_pose_history)>2 and len(copy_pose_time_history)>0:
            ...
            inputs = []
            for idx in range(len(braking_forces)):
                modified_U_0 = copy.deepcopy(U_0)
                inputs.append([TIME_STEPS_TO_CALCULATE,0,constants.REACHABILITY_DT,constants.MODEL_SUBTIME_STEPS,X_0,sigma_0,modified_U_0,braking_forces[idx]])
            probabilities_of_collision  = fast_pool.map(collision_probability_modifications.breaking_example, inputs)
...
def animate(i):
    ...
    global_subplot.set_title("Probability of Collision For Braking Forces")
