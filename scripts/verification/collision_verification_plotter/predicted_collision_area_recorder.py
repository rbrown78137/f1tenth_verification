...
PROBABILITY_SQUARE_DISTANCE_REFINEMENT = 0.02
...
def draw_centers(X_0, sigma_0, U_0, ax):
    probstars = collision_probability.probstar_next_k_time_steps_given_initial_state(FUTURE_TIME_STEPS,0,constants.REACHABILITY_DT,constants.MODEL_SUBTIME_STEPS,X_0,sigma_0,U_0,6)
    # Initial Position
    ax.plot([X_0[0]], [X_0[1]], marker="s", markersize=MARKER_SIZE, markeredgecolor=(0,0,0), markerfacecolor=(1.0,1.0,1.0)) 
    ax.plot([X_0[4]], [X_0[5]], marker="o", markersize=MARKER_SIZE, markeredgecolor=(0,0,0), markerfacecolor=(1,1.0,1.0))
    ...
    for star_idx,star in enumerate(probstars):
        ...
        ax.plot([x_omega], [y_omega], marker="o", markersize=MARKER_SIZE, markeredgecolor=(0,0,0), markerfacecolor=(1,1,1.0))
    ...
    for timestep_idx in range(0,FUTURE_TIME_STEPS):
        ...
        for i in range(image_width_large_pass):
            for j in range(image_height_large_pass):
                ...
                probstars = collision_probability.probstar_next_k_time_steps_given_initial_state_bounded_pi(X_MIN,X_MAX,Y_MIN,Y_MAX,FUTURE_TIME_STEPS,0,constants.REACHABILITY_DT,constants.MODEL_SUBTIME_STEPS,X_0,sigma_0,U_0,standard_deviations=6)
                # probstars = collision_probability.probstar_next_k_time_steps_given_initial_state(FUTURE_TIME_STEPS,0,constants.REACHABILITY_DT,constants.MODEL_SUBTIME_STEPS,X_0,sigma_0,U_0,standard_deviations=6)
...
def get_graph_instance(pose_history,actuation_history,pose_time_history,reachability_dt=0.25,model_sub_time_steps=10):
    global VIEW_X_MIN,VIEW_X_MIN,VIEW_Y_MIN,VIEW_Y_MAX
    ...
    ax.set_title("Predicted Movement of Both Cars at Current Time Step",fontdict={'family' : 'normal',
        'weight' : 'bold',
        'size'   : 18})
