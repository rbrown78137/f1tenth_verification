...
def animate(i):
    ...
    global_subplot.set_xlabel("Time (S)")
    global_subplot.set_title("Probability of Collision in Next 5 Time Steps")
    if len(probability_collision_history)>0 and len(probability_collision_history[0])>0:
        ...
        for future_timestep_idx in range(len(probability_collision_history[0][1])):
            x = [(item[0]-probability_collision_history[0][0])* REPLAY_SPEED for item in probability_collision_history]
